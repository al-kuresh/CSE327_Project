...
if (!isset($_SESSION['mobile_recharge'])) {
    header("Location: mobile_recharge.php");
    exit;
}

$data = $_SESSION['mobile_recharge'];

if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['confirm']) && $_POST['confirm'] == 'yes') {
        if ($data['amount'] > 0 && $data['amount'] <= $transactionManager->getBalance($_SESSION['user_id'])) {
            $result = $transactionManager->processTransaction($_SESSION['user_id'], 'mobile_recharge', $data['amount'], $data['phone']);
            if ($result['success']) {
                // Update session balance
                $_SESSION['balance'] = $transactionManager->getBalance($_SESSION['user_id']);
                unset($_SESSION['mobile_recharge']);
                $message = "Recharge successful! " . $result['payment'];
...
<head>
    ...
    <title>Confirm Mobile Recharge</title>
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <form method="POST">
                        ...
                        <a href="mobile_recharge.php" class="btn btn-secondary">Cancel</a>
                    </form>
                </div>
            </div>
        <?php endif; ?>
